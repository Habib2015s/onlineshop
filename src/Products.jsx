    ...
    return(
        <div className="m-auto w-3/4">
            {isPending ? ("loading..."):(
                ...
                <div onClick={()=>{
                    setisOpen(true)}} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mb-10 items-stretch">
                   
                    
                    {data.data.map((item)=>{return(
                        
                        
                        <div key={item.id}   className="w-[100%] h-[100%] flex flex-col gap-y-3 bg-white p-2 shadow rounded-md hover:scale-101 cursor-pointer transition-transform relative        ">
            ...
            <SideBarModal visible={isOpen} onclose={handleCloseModal} />
